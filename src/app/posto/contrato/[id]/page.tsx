...
export default function ContratoDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!contrato) {
    return (
      <div style={{ padding: 32 }}>
        <Card><p style={{ fontFamily: 'var(--font-body)', color: 'var(--tanqe-gray)' }}>Contrato não encontrado.</p></Card>
      </div>
    )
  }
  ...
  return (
    <div style={{ display: 'grid', gap: 24 }}>
      <Link href="/posto/contratos" style={{ fontFamily: 'var(--font-mono)', fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.12em', color: 'var(--tanqe-gray)', textDecoration: 'none' }}>← Voltar pra contratos</Link>

      <SectionHeader
        eyebrow={`Contrato ${contrato.numero}`}
        title="Acordo de fornecimento"
        subtitle="Documento jurídico vinculante entre posto e distribuidora, gerado automaticamente após o leilão."
        action={
      ...
      <div style={{ background: 'var(--tanqe-white)', border: '1px solid var(--tanqe-stone)', borderRadius: 4, padding: '48px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 32, marginBottom: 32 }}>
          ...
          <PartieBlock title="Contratada" entity={dist} role="Distribuidora" />
        </div>
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          <div style={{ background: 'var(--tanqe-cream)', padding: 20, borderRadius: 4, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 16 }}>
            ...
            <KV label="Valor total" value={formatBRL(contrato.valor)} highlight />
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          <ol style={{ paddingLeft: 24, margin: 0 }}>
            {contrato.clausulas.map((c, i) => (
              <li key={i} style={{ fontFamily: 'var(--font-body)', fontSize: 14, color: 'var(--tanqe-black)', lineHeight: 1.7, marginBottom: 10 }}>
                {c}
              </li>
            ))}
          </ol>
        ...
        <div style={{ paddingTop: 24, borderTop: '1px solid var(--tanqe-stone)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 24 }}>
            <SignatureBlock title="Posto" entityName={posto?.nome || '—'} signedAt={contrato.assinadoPostoEm} fallbackAction={
              contrato.status !== 'assinado' && !signed && contrato.assinadoPostoEm === undefined ? (
                <Button onClick={() => setSigned(true)} icon={Check} size="sm">Assinar agora</Button>
              ) : null
            } overrideSigned={signed} />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 16 }}>
        {nfe && (
          <Card>
            ...
            <Link href="/posto/nfes" style={{ fontFamily: 'var(--font-mono)', fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.12em', color: 'var(--tanqe-orange)', textDecoration: 'none', display: 'inline-block', marginTop: 8 }}>ver detalhes →</Link>
          </Card>
        )}
        {pagamento && (
          <Card>
            ...
            <p style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 18, margin: 0 }}>{pagamento.status === 'pago' ? 'Pago' : 'Pendente'}</p>
        ...
        {pedido && (
          <Card>
            <p style={{ fontFamily: 'var(--font-mono)', fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.15em', color: 'var(--tanqe-gray)', margin: 0, marginBottom: 6 }}>Pedido</p>
            <p style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 18, margin: 0 }}>{pedido.status === 'entregue' ? 'Entregue' : 'Em trânsito'}</p>
            <Link href="/posto/pedidos" style={{ fontFamily: 'var(--font-mono)', fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.12em', color: 'var(--tanqe-orange)', textDecoration: 'none', display: 'inline-block', marginTop: 8 }}>rastrear →</Link>
          </Card>
        )}
      </div>
    </div>
...
interface PartieBlockProps {
  title: string
  entity?: { nome: string; razaoSocial: string; cnpj: string; endereco: string; cidade: string; uf: string }
  role: string
}

function PartieBlock({ title, entity, role }: PartieBlockProps) {
  return (
    <div>
...
function SignatureBlock({ title, entityName, signedAt, fallbackAction, overrideSigned }: { title: string; entityName: string; signedAt?: string; fallbackAction?: React.ReactNode; overrideSigned?: boolean }) {
  const isSigned = !!signedAt || overrideSigned
  return (
      ...
      {isSigned ? (
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div>
            ...
            <p style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--tanqe-gray)', margin: 0 }}>{signedAt ? formatDataHora(signedAt) : 'agora'}</p>
          </div>
        </div>
      ) : (
        <>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: fallbackAction ? 12 : 0 }}>
